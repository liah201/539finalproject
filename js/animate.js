...
function updatePreviewImage(index) {
    if (currentImageIndex !== index) {
      ...
      const targetImage = itemImages[index];
      const targetSrc = targetImage.getAttribute('src');
      previewImage.setAttribute('src', targetSrc);
  
      // ✅ Update caption text
    const desktopCaption = document.getElementById("img-caption");
    const mobileCaption = document.getElementById("img-caption-mobile");

        if (desktopCaption) desktopCaption.textContent = imageCaptions[index] || "";
        if (mobileCaption) mobileCaption.textContent = imageCaptions[index] || "";
    }
}
...
window.addEventListener("resize", () => {
    ...
    targetTranslate = Math.min(targetTranslate, -newMaxTranslate);
